...
public abstract class AbstractHeaderParser implements HeaderParser {
    ...
    static final String Flag_Wechat = "Wechat";
...
//                }
            ...
            } else if (value.startsWith(Flag_Wechat)) {
                String authHeaderValue = value.substring(Flag_Wechat.length()).trim();
                int commaIndex = authHeaderValue.indexOf(',');
                if (commaIndex > 0) {
                    authHeaderValue = authHeaderValue.substring(0, commaIndex);
                }
                return new WechatHeaderParser(authHeaderValue);
            }
        }
        return null;
    }
